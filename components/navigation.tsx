...
export function Navigation() {
  ...
  return (
    <nav className="fixed left-0 top-0 z-50 flex h-full w-64 flex-col justify-between border-r border-border bg-background/95 p-8 backdrop-blur-md max-lg:hidden">
      <div>
        <div className="mb-12">
          ...
          {navItems.map(({ id, label }) => (
            <li key={id}>
              <button
                ...
                className={`group flex items-center gap-3 text-sm font-medium transition-colors cursor-pointer ${
                ...
                <span
                  className={`h-px transition-all ${
                    activeSection === id
                      ...
                      : "w-8 bg-muted-foreground group-hover:w-16 group-hover:bg-gradient-to-r group-hover:from-primary group-hover:to-accent"
                  }`}
                />
                {label}
              </button>
